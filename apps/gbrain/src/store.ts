...
export interface GraphData {
  nodes: { id: string; label: string; type: PageType }[];
  edges: GraphEdge[];
}
...
export class GbrainStore {
  pages = new Map<string, KnowledgePage>();
  ...
  queryPages(query: string): KnowledgePage[] {
    ...
    const scored: { page: KnowledgePage; score: number }[] = [];
    ...
    for (const page of this.pages.values()) {
      ...
      if (score > 0) {
        ...
        // Confidence bonus
        if (page.confidence === 'verified') score += 3;
  ...
  seed(): void {
    if (this.pages.size > 0) return; // Already seeded

    const decision = this.createPage({
      title: 'Use Fastify as our API framework',
      content:
        'After evaluating Express, Fastify, and Hono, the team decided to adopt Fastify for all new API services. Fastify provides better performance benchmarks, built-in JSON schema validation, and a mature plugin ecosystem. Migration from existing Express services will be completed by Q3.',
      type: 'decision',
    ...
    const plan = this.createPage({
      ...
      content:
        'The Q3 product launch targets enterprise customers with the v1.0 release of AIGENCY OS. Key milestones: July 15 - feature freeze, August 1 - beta release to 10 design partners, September 1 - GA release. Revenue target: $500K ARR by end of Q3. Marketing budget allocated: $50K for launch events and content.',
      type: 'plan',
    ...
    const auditFinding = this.createPage({
      ...
      content:
        'The Q2 security audit identified that authentication endpoints (/login, /register, /reset-password) lack rate limiting. This exposes the system to brute-force attacks. Severity: HIGH. Recommended fix: implement rate limiting with a sliding window of 10 requests per minute per IP. Fix deadline: July 1, 2026.',
      type: 'audit_finding',
    ...
    const assumption = this.createPage({
      ...
      content:
        'Based on early user interviews (n=25), 72% of prospects indicated a preference for self-serve product tours over scheduling a sales call. This assumption will be validated with the beta launch by tracking the self-serve-to-paid conversion rate. If conversion exceeds 5%, the assumption holds.',
      type: 'assumption',
