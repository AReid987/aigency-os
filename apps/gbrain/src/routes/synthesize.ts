  ...
  app.post('/api/v1/synthesize', async (request, reply) => {
    ...
    if (findings.length > 0) {
      recommendations.push(
        `Address ${findings.length} open audit finding(s) before the next release.`,
      );
    }
    if (assumptions.length > 0) {
      recommendations.push(
        `Validate ${assumptions.length} unverified assumption(s) through targeted experiments.`,
      );
    }
    if (decisions.length > 0) {
      recommendations.push(
        `Review ${decisions.length} architectural decision(s) for alignment with current strategy.`,
      );
    }
    if (plans.length > 0) {
      recommendations.push(
        `Track progress on ${plans.length} active plan(s) and update milestones.`,
      );
    }
