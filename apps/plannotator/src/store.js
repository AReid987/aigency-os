...
class PlannotatorStore {
    plans = new Map();
    annotations = new Map(); // planId -> annotations
    versions = new Map(); // planId -> versions
    // ─── Plan Methods ───────────────────────────────────────────────────────
    createPlan(data) {
    ...
    getPlan(id, role) {
        ...
        const filteredSections = plan.sections.filter((section) => {
            return section.accessRoles.includes(role);
        });
    ...
    getAnnotations(planId) {
        return this.annotations.get(planId) ?? [];
    }
    // ─── Diff Methods ───────────────────────────────────────────────────────
    getDiff(planId, v1, v2) {
