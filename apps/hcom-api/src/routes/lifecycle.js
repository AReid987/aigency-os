...
export async function lifecycleRoutes(app) {
    // Spawn new agent
    app.post('/hcom/agents/:id/spawn', async (request, reply) => {
        ...
        if (!parent)
            return reply.code(404).send({ error: 'Parent agent not found' });
        ...
        insertAgentRow({
            ...
            adapter: (parsed.data.adapter ?? parent.adapter),
    ...
    app.post('/hcom/agents/:id/fork', async (request, reply) => {
        ...
        if (!source)
            return reply.code(404).send({ error: 'Source agent not found' });
        const newId = crypto.randomUUID();
        const count = countAgents.get().count;
        const sessionId = `tmux:${count}`;
        ...
        if (parsed.data.shareContext) {
            ...
            // Copy subscriptions
            const subs = selectAgentSubscriptions.all(sourceId);
