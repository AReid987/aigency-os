// ─── Domain Types ──────────────────────────────────────────────────────────
// ─── PRD Parser ────────────────────────────────────────────────────────────
function generateTasksFromPRD(prd) {
    ...
    const tasks = selected.map((t, idx) => ({
...
class Store {
    ...
    unifyPlan(id) {
        ...
        const passedCriteria = plan.tasks.reduce((acc, t) => acc + t.acceptanceCriteria.filter((c) => c.status === 'passed').length, 0);
    ...
    updateCriteriaStatus(planId, criteriaId, status) {
        const plan = this.plans.get(planId);
        if (!plan)
            return undefined;
        for (const task of plan.tasks) {
