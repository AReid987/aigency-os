...
export interface RemediationTask {
  ...
  riskScore: number; // severity * confidence → 0-1
...
const DOMAIN_FINDINGS: Record<Domain, Array<{ title: string; description: string; severity: Severity; confidence: number; recommendation: string }>> = {
  security: [
...
function riskScoreToPriority(score: number): RemediationTask['priority'] {
...
function riskScoreToEffort(score: number): RemediationTask['effort'] {
...
class AegisStore {
  ...
  personaEvaluations = new Map<string, PersonaEvaluation[]>(); // auditId -> evaluations
  remediationPlans = new Map<string, RemediationPlan>(); // auditId -> plan

  createAudit(data: { project: string; domains?: Domain[] }): Audit {
  ...
  transformToRemediation(auditId: string): RemediationPlan | undefined {
    ...
    const tasks: RemediationTask[] = audit.findings.map(f => {
  ...
  getExecutiveSummary(auditId: string): ExecutiveSummary | undefined {
    ...
    let personaConsensus: string | null = null;
  ...
  addPersonaEvaluation(auditId: string, data: {
    ...
    if (!this.audits.has(auditId)) return undefined;

    const evaluation: PersonaEvaluation = {
      id: crypto.randomUUID(),
      auditId,
