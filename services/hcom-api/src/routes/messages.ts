...
export async function messageRoutes(app: FastifyInstance) {
  // Send message
  app.post<{ Body: z.infer<typeof SendMessageSchema> }>('/hcom/agents/:id/message', async (request, reply) => {
    ...
    if (!sender) return reply.code(404).send({ error: 'Sender agent not found' });
    ...
    if (recipientId) {
      ...
      if (!recipient) return reply.code(404).send({ error: 'Recipient agent not found' });
    }
